...
class InvalidPublicPostIdError(ValueError):
    pass
...
def encode_post_id(post_id: int) -> str:
...
def decode_post_id(public_id: str) -> int:
    if not public_id.startswith(PUBLIC_ID_PREFIX):
        raise InvalidPublicPostIdError("Invalid post id")
    ...
    try:
        ...
        raise InvalidPublicPostIdError("Invalid post id") from error
