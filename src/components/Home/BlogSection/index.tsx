...
export default function BlogSection({ index, item, posts }: BlogSectionProps) {
  ...
  const recommendedPosts = floatingPosts.slice(0, 5)
      ...
      <div className="relative max-w-6xl mx-auto -ml-6 sm:-ml-10 lg:-ml-16">
        <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(320px,380px)]">
          ...
          {recommendedPosts.length > 0 && (
            <aside className="flex h-full items-center pl-6 sm:pl-10 lg:pl-0">
              <div className="w-full">
                <p className="mb-3 px-3 text-sm font-semibold text-[var(--site-text-muted)]">
                  {t('recommendedForYou', { default: '推荐你看' })}
                </p>
                <div className="divide-y divide-[var(--site-border)]">
                  {recommendedPosts.map((post) => (
                    <Link
                      ...
                      href={`/posts/${post.id}`}
                      className={cn(
                        'group/post flex items-center gap-3 px-3 py-3',
                        'focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[var(--site-focus-ring)]'
                      )}
                    >
                      <PostIcon className="h-4 w-4 flex-shrink-0 text-[var(--site-text-tertiary)] transition-colors group-hover/post:text-[var(--site-action)]" />
                      <span className="min-w-0 flex-1 truncate text-sm font-semibold text-[var(--site-text)] transition-colors group-hover/post:text-[var(--site-action)]">
                        {post.title}
                      </span>
                      <time
                        dateTime={post.createdAt}
                        className="flex-shrink-0 text-xs text-[var(--site-text-tertiary)] transition-colors group-hover/post:text-[var(--site-action)]"
                      >
